...
class SelectActionMixin(MultipleObjectMixin):
    ...
    def get_selection(self, queryset=None):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "model": self.model,
                "action_form": self.get_action_form(),
                "action_form_url": self.get_action_form_url(self.request),
                "actions_selection_counter": self.actions_selection_counter,
                "selected": self.get_selection(queryset),
            }
        )
        return context
...
class ListFilterMixin(MultipleObjectMixin):
    ...
    list_filter = None
    list_filter_template = "admin/filter.html"
    ...
    def setup(self, request, *args, **kwargs):
        ...
        if not hasattr(self, "model_admin"):
            self.model_admin = self.modeladmin_class(self.model, AdminSite())
...
class VariableModelMixin:
    models = None
    app_config: AppConfig = None
    model_lookup_kwarg = "model_name"

    def setup(self, *args, **kwargs):
        if getattr(self, "model", None) is None:
            app_label = self.app_config.name.split(".")[-1]
            try:
                self.model = apps.get_model(
                    app_label=app_label,
                    model_name=kwargs[self.model_lookup_kwarg],
                )
                if self.models:
